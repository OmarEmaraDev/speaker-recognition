...
def loadSpeakerFiles():
    ...
    shuffle(speakerFiles)
    trainingDataLength = int(len(speakerFiles) * 0.85)
    trainingSpeakerFiles = speakerFiles[:trainingDataLength]
    testingSpeakerFiles = speakerFiles[trainingDataLength:]
    return trainingSpeakerFiles, testingSpeakerFiles

def loadMFCCData():
    trainingSpeakerFiles, testingSpeakerFiles = loadSpeakerFiles()
    trainingSamples = numpy.stack([mfcc(f.samples, f.samplingRate) for f in trainingSpeakerFiles])
    trainingLabels = numpy.fromiter((f.index for f in trainingSpeakerFiles), dtype = numpy.uint8)
    testingSamples = numpy.stack([mfcc(f.samples, f.samplingRate) for f in testingSpeakerFiles])
    testingLabels = numpy.fromiter((f.index for f in testingSpeakerFiles), dtype = numpy.uint8)
